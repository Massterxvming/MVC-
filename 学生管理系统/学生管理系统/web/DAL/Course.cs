...
namespace DBA.DAL
{
	...
	public partial class Course
	{
		...
		public DBA.Model.Course GetModel(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,CourseName from Course ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public DBA.Model.Course DataRowToModel(DataRow row)
		{
			DBA.Model.Course model=new DBA.Model.Course();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,CourseName ");
