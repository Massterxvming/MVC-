...
namespace Conris.Utility
{
    public class DbHelperSQL
    {
        ...
        private static string getConnectionString()
        {
            ...
            ConnectionStringSettings setting = ConfigurationManager.ConnectionStrings["YBJKConnectionString"];
        ...
        /// </summary>
        /// <returns></returns>
        public static bool IsConnection(string sqlconnectionString)
        {
        ...
        /// <param name="content">参数内容,比如一个字段是格式复杂的文章，有特殊符号，可以通过这个方式添加</param>
        /// <returns>影响的记录数</returns>
        public static object ExecuteSqlGetSingle(string SQLString, string content)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand cmd = new SqlCommand(SQLString, connection);
        ...
        public static int ExecuteSql(string SQLString, params SqlParameter[] cmdParms)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    try
                    {
                        PrepareTextCommand(cmd, connection, null, SQLString, cmdParms);
                        int rows = cmd.ExecuteNonQuery();
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        foreach (DictionaryEntry myDE in SQLStringList)
                        {
                            ...
                            PrepareTextCommand(cmd, conn, trans, cmdText, cmdParms);
        ...
        public static void PrepareTextCommand(SqlCommand cmd, SqlConnection conn, SqlTransaction trans, string cmdText, SqlParameter[] cmdParms)
        {
        ...
        /// <summary>
        /// 执行存储过程，返回DataReader(使用该方法切记要手工关闭SqlDataReader和连接)
        /// </summary>
        ...
        public static DataSet GetPageList(int PageIndex, int PageSize, string Table, string Field, string OrderBy, string PK, string Filter, ref int MaxPage, ref int TotalRow, ref string Descript)//out是在外面不能赋值的
        {
            ...
            int.TryParse(pars[7].Value==null?"0":pars[7].Value.ToString(), out MaxPage);
            int.TryParse(pars[8].Value == null ? "0" : pars[8].Value.ToString(), out TotalRow);
            //MaxPage = (int)pars[7].Value;
            //TotalRow = (int)pars[8].Value;
            Descript = pars[9].Value==null?"":pars[9].Value.ToString();
            return ds;
        }
